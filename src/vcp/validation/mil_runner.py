...
def _reference_row_fields(
    profile: SyntheticReferenceProfile | CommonRoadReferencePath,
    state: VehicleState,
    time_s: float,
) -> dict[str, float | str]:
    if not isinstance(profile, CommonRoadReferencePath):
        return {
            "reference_progress_m": "",
            "reference_remaining_distance_m": "",
        }

    position = np.array([state.px, state.py], dtype=np.float64)
    progress_m = project_reference_path_progress(profile, position)
    sample = sample_reference_path_at_progress(profile, progress_m)
    return {
        "reference_progress_m": progress_m,
        "reference_remaining_distance_m": sample.remaining_distance_m,
    }
...
def _row(
    *,
    ...
    reference_fields = _reference_row_fields(spec.reference_profile, state, time_s)
    return {
...
def _write_report(results: list[MILRunResult], output_path: Path) -> None:
    lines = [
        ...
        "",
        "| Scenario | Controller | Source | Result | Success | Lateral RMSE | "
